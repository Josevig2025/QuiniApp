...
export default function FilterBar({ filters, onApply, minFecha, maxFecha }) {
  ...
  useEffect(() => { disparar() }, [cifras, hasta, rango, ultN, posExacta, JSON.stringify(diaSemana), JSON.stringify(diaMes)]) // eslint-disable-line
  ...
  const hayAvanzados = diaSemana.length > 0 || diaMes.length > 0
      ...
      <button onClick={() => setExpandido(p => !p)}
        ...
        {hayAvanzados && (
          <span className="px-1.5 py-0.5 rounded-full text-xs font-bold"
            ...
            {diaSemana.length + diaMes.length}
          </span>
        )}
      </button>
