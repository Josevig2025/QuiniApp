...
import {
  ...
  Zap, Crown, Search, ArrowUp, ArrowDown, Clock,
} from 'lucide-react'
...
export default function Estadisticas() {
  ...
  // ── Fetch ────────────────────────────────────────────────────────────
  // Cuando se selecciona 'recientes', ordenar por gap_actual ascendente automáticamente
  useEffect(() => {
    if (tab === 'recientes') {
      setSortCol('gap_actual')
      setSortDir('asc')
  ...
  const cargar = useCallback(async (tabId = tab) => {
    ...
    try {
      ...
      switch (tabId) {
        ...
        case 'recientes':   res = await getRanking(filters.cifras, filters.hasta, 'atrasos',    top, params); break
  ...
  const buildRows = (tabId) => {
    const allRows = sortData(data[tabId])
    if (!allRows) return []
  ...
  const DESCRIPCIONES = {
    ...
    recientes:   `Números que salieron recientemente — menor gap actual. Ordenados por gap ascendente.`,
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 pb-24 md:pb-6 space-y-4">
      ...
      {/* Aviso si hay más de 200 filas */}
      {!loading[tab] && !errors[tab] && data[tab]?.length > 200 && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs"
          style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)' }}>
          <span className="text-amber-400">⚠</span>
          <span style={{ color: 'var(--text-secondary)' }}>
            Mostrando primeros 200 de {data[tab].length} resultados.
            Usá el orden por columna para ver los más relevantes.
          </span>
        </div>
      )}
