...
export default function QuinielaMain() {
  ...
  // Ref para siempre tener el toParams más reciente (incluye diaSemana/diaMes)
  ...
  const cargarRanking = useCallback(async () => {
    ...
    try {
      ...
      const params = f.toParamsConPos?.() ?? f.toParams()
      const [rA, rS, rR, rG] = await Promise.all([
        getRanking(f.cifras, f.hasta, 'atrasos',    20, params),
        getRanking(f.cifras, f.hasta, 'frecuencia', 20, params),
        getRacha(f.cifras, f.hasta, 10, params),
        getGapsCortos(f.cifras, f.hasta, 20, params),
      ])
  ...
  }, []) // sin dependencias — usa siempre filtersRef.current

  useEffect(() => { cargarRanking() }, []) // eslint-disable-line
