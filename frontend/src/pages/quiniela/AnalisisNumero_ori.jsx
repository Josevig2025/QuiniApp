      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex items-end gap-3 flex-1">
          <div
            ...
            style={{ fontSize: 'clamp(2.5rem, 8vw, 4rem)', color: headerColor }}
          >
            {numero}
          </div>
          {gapActual != null && (
            <div className="mb-1">
              <p className="text-xs text-muted">Gap actual</p>
              <GapBadge value={gapActual} className="text-sm" />
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-1">
          <button onClick={irAnterior} disabled={idxAct <= 0}
            className="btn-ghost p-1.5 disabled:opacity-30" title="Número anterior">
            <ArrowLeft className="w-4 h-4" />
          </button>
          <button onClick={irSiguiente} disabled={idxAct >= lista.length - 1}
            className="btn-ghost p-1.5 disabled:opacity-30" title="Número siguiente">
            <ArrowRight className="w-4 h-4" />
          </button>
          <button onClick={irAzar} className="btn-ghost p-1.5" title="Número al azar">
            <Shuffle className="w-4 h-4" />
          </button>
          <button onClick={() => cargar(numero)}
            className="btn-ghost p-1.5" title="Actualizar">
      ...
      <div className="flex gap-4 items-start">
        <div className="flex flex-col items-center gap-2 flex-1">
          <input
            ...
            type="text"
            value={inputNum}
            onChange={e => setInputNum(e.target.value.replace(/[^0-9]/g,'').slice(0, filters.cifras))}
            onKeyDown={e => e.key === 'Enter' && irManual()}
            placeholder={filters.cifras === 2 ? '00' : filters.cifras === 3 ? '000' : '0'}
            style={{
              width: 110, height: 88, fontSize: '2.6rem',
              fontFamily: 'var(--font-display)', fontWeight: 'black',
              textAlign: 'center', borderRadius: 20, outline: 'none',
              background: inputNum ? 'var(--bg-card)' : 'var(--bg-elevated)',
              border: inputNum ? '3px solid rgba(0,229,255,0.6)' : '2px solid var(--border)',
              color: inputNum ? 'var(--accent-cyan)' : 'var(--text-muted)',
              boxShadow: inputNum ? '0 0 0 4px rgba(0,229,255,0.1)' : 'none',
              transition: 'all 0.15s',
            }}
          />
          <div className="flex gap-2">
            <button onClick={irAzar}
              className="btn-ghost flex items-center gap-1.5 px-3 text-xs"
              title="Número al azar">
              <Shuffle className="w-3.5 h-3.5" /> Azar
            </button>
          </div>
        </div>
