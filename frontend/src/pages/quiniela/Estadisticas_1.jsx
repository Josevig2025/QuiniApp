...
import {
  getRanking, getGapsCortos, getRacha, getLideres, getInfo,
} from '../../api/quiniela'
    ...
    <div className="card p-6 space-y-4 max-w-sm mx-auto mt-4">
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder={cifras === 2 ? 'ej: 07' : cifras === 3 ? 'ej: 007' : 'ej: 7'}
          className="field flex-1 text-center font-display text-2xl py-3"
          style={{ color: 'var(--accent-cyan)' }}
          maxLength={cifras}
          autoFocus
        />
        <button onClick={ir} className="btn-primary px-5">
          Ir →
        </button>
      </div>
...
export default function Estadisticas() {
  ...
  // ── Filas por tab ─────────────────────────────────────────────────────
  const MAX_RENDER = 200  // más de 200 filas colapsa el DOM

  const buildRows = (tabId) => {
    const rows = sortData(data[tabId])
    if (!rows) return []
    const rowsLimitadas = rows.slice(0, MAX_RENDER)
