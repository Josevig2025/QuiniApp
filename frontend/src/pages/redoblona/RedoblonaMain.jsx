  ...
  return (
    <div className="card p-3 space-y-2">
      <div className="flex flex-wrap gap-x-4 gap-y-2 items-end">
        {/* Rango premios */}
        <div>
          <p className="text-xs text-muted mb-1">N1 hasta pos.</p>
        ...
        <div>
          <p className="text-xs text-muted mb-1">N2 hasta pos. (mín.5)</p>
...
function TablaPagos({ r1, r2 }) {
  const pago = r1 && r2 ? PAGOS_REDOBLONA[r1]?.[r2] : null

  // Mostrar ventana contextual: ±3 filas y ±3 columnas alrededor de r1/r2
  const r1c = r1 || 5
  const r2c = r2 || 5
  const r1Min = Math.max(1,  r1c - 3)
  const r1Max = Math.min(20, r1c + 3)
  const r2Min = Math.max(1,  r2c - 3)
  const r2Max = Math.min(20, r2c + 3)
  const filas = Array.from({length: r1Max - r1Min + 1}, (_, i) => r1Min + i)
  const cols  = Array.from({length: r2Max - r2Min + 1}, (_, i) => r2Min + i)
  ...
  return (
    <div className="rounded-xl overflow-hidden flex-shrink-0"
      ...
      <div className="px-3 py-1.5 text-center"
        ...
        <p className="text-xs text-muted">N1 top{r1||'?'} · N2 top{r2||'?'}</p>
      ...
      <div className="p-1.5 overflow-auto">
        <table style={{ borderCollapse: 'collapse', fontSize: '0.58rem' }}>
          <thead>
            <tr>
              <th style={{ padding: '2px 4px', color: 'var(--text-muted)', fontWeight: 500 }}></th>
              {cols.map(c => (
                <th key={c} style={{
                  padding: '2px 4px', textAlign: 'center',
                  color: c === r2 ? 'var(--accent-cyan)' : 'var(--text-muted)',
                  fontWeight: c === r2 ? 700 : 400,
                  background: c === r2 ? 'rgba(0,229,255,0.08)' : 'transparent',
                }}>{c}</th>
              ))}
            </tr>
          </thead>
            ...
            {filas.map(fila => (
              <tr key={fila}>
                <td style={{
                  ...
                  color: fila === r1 ? 'var(--accent-cyan)' : 'var(--text-muted)',
                  background: fila === r1 ? 'rgba(0,229,255,0.08)' : 'transparent',
                }}>{fila}</td>
                {cols.map(c => {
                  const val = PAGOS_REDOBLONA[fila]?.[c]
                  const activo = fila === r1 && c === r2
                  return (
                    <td key={c} style={{
                      padding: '2px 4px', textAlign: 'center', fontFamily: 'monospace',
                      background: activo ? 'rgba(0,229,255,0.25)'
                        : (fila === r1 || c === r2) ? 'rgba(0,229,255,0.05)' : 'transparent',
                      color: activo ? 'var(--accent-cyan)'
                        : val != null ? 'var(--text-secondary)' : 'var(--text-muted)',
                      fontWeight: activo ? 700 : 400,
                      border: activo ? '1px solid rgba(0,229,255,0.5)' : '1px solid transparent',
                      borderRadius: activo ? 3 : 0,
                    }}>
                      {val != null ? val : '·'}
                    </td>
                  )
        ...
        <p style={{ fontSize: '0.5rem', color: 'var(--text-muted)', textAlign: 'center', marginTop: 2 }}>
          fila=N1 · col=N2
        </p>
      </div>
    </div>
...
export default function RedoblonaMain() {
  ...
  const [filtros, setFiltros] = useState({
    r1: 5, r2: 5, rango: 'todo', ultN: 300, desde: '', hastaF: ''  // r2 mínimo 5 por regla del juego
  })
