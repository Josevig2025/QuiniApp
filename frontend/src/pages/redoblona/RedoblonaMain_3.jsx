  ...
  return (
    <div className="card p-3 space-y-2">
      <div className="flex flex-wrap gap-x-4 gap-y-2 items-end">
        {/* Rango premios */}
        <div>
          <p className="text-xs text-muted mb-1">Premios N1</p>
        ...
        <div>
          <p className="text-xs text-muted mb-1">Premios N2</p>
...
function TablaPagos({ r1, r2 }) {
  ...
  const tieneValores = r1 >= 1 && r1 <= 20 && r2 >= 5 && r2 <= 20
  ...
  return (
    <div className="rounded-xl overflow-hidden flex-shrink-0"
      ...
      <div className="px-3 py-2 text-center"
        ...
        <p className="text-xs font-semibold uppercase tracking-wider text-muted">Pago</p>
        {pago != null ? (
          ...
          <p className="text-xs text-muted mt-1">
            {tieneValores ? 'Sin premio' : 'Ingresá posiciones'}
          </p>
        )}
      </div>
      <div className="p-2 space-y-1">
        <p className="text-xs text-muted text-center mb-1.5">
          N1 pos <span style={{ color: r1 ? 'var(--accent-cyan)' : 'var(--text-muted)' }}>{r1 || '?'}</span>
          {' · '}
          N2 pos <span style={{ color: r2 >= 5 ? '#A78BFA' : 'var(--text-muted)' }}>{r2 || '?'}</span>
        </p>
        {/* Mini tabla visual de rangos disponibles */}
        <div className="grid grid-cols-4 gap-0.5">
          {[5,6,7,8,9,10,12,15,20].map(c => {
            const activo = c === r2
            const disponible = PAGOS_REDOBLONA[r1]?.[c] != null
            return (
              <div key={c}
                className="text-center py-1 rounded text-xs font-mono-custom"
                style={{
                  background: activo
                    ? 'rgba(167,139,250,0.3)'
                    : disponible ? 'var(--bg-elevated)' : 'transparent',
                  color: activo ? '#A78BFA'
                    : disponible ? 'var(--text-secondary)' : 'var(--text-muted)',
                  border: activo ? '1px solid #A78BFA' : '1px solid transparent',
                  fontSize: '0.65rem',
                }}>
                {disponible ? c : ''}
              </div>
        ...
        <p className="text-xs text-muted text-center" style={{ fontSize: '0.6rem' }}>
          Posibles N2 para N1={r1 || '?'}
        </p>
      </div>
    </div>
  )
