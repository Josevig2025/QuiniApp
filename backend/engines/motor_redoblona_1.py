...
class MotorRedoblona:
    ...
    def ranking(
        ...
        for idx in range(idx_min, idx_max + 1):
            ...
            # Números dentro de r1 y r2 respectivamente
            set_r1 = set(reducidos[:min(r1, len(reducidos))])
            set_r2 = set(reducidos[:min(r2, len(reducidos))])

            # Generar todos los pares (a, b) con a < b que están en ambos rangos
            comunes = sorted(set_r1 & set_r2)
            for i in range(len(comunes)):
                for j in range(i + 1, len(comunes)):
                    par = (comunes[i], comunes[j])
