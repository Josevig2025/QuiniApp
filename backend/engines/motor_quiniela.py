...
def _parse_fecha_flexible(fecha_str: str) -> datetime:
    """Acepta DD/MM/YY, DD/MM/YYYY o YYYY-MM-DD (formato ISO del input date)."""
    s = fecha_str.strip()
...
class MotorQuiniela:
    ...
    def analizar_numero(
        ...
        pos_exacta: int | None = None,   # si se indica, solo cuenta esa posición
    ) -> dict:
        """
        Análisis completo de un número: apariciones, gaps, frecuencia por posición.
        pos_exacta: si se indica (1-20), solo cuenta apariciones en esa posición exacta.
        """
        ...
        for idx in range(idx_min, idx_max + 1):
            ...
            if pos_exacta is not None:
                # Solo buscar en la posición exacta
                pos = pos_exacta - 1
                if pos < len(nums):
        ...
        return {
            ...
            "todas_fechas": [a["fecha"] for a in apariciones],
        }
    ...
    def _indices_por_dia(
        self,
        idx_min: int,
        idx_max: int,
        dia_semana: list[int] | None = None,  # 0=lun..6=dom
        dia_mes:    list[int] | None = None,  # 1..31
        mes:        list[int] | None = None,  # 1=ene..12=dic
    ) -> set[int] | None:
        """
        Devuelve el conjunto de índices que corresponden a los filtros de día/mes.
        Si no hay ningún filtro, devuelve None.
        """
        if not dia_semana and not dia_mes and not mes:
            return None
        from datetime import datetime
        validos = set()
        for i in range(idx_min, idx_max + 1):
            fecha_str = self.historial[i][0]
            try:
                partes = fecha_str.strip().split()
                dt = datetime.strptime(partes[0], "%d/%m/%y")
                if dia_semana and (dt.weekday() not in dia_semana):
                    continue
                if dia_mes and (dt.day not in dia_mes):
                    continue
                if mes and (dt.month not in mes):
                    continue
                validos.add(i)
            except Exception:
                pass
    ...
    def ranking(
        ...
        idx_min: int | None = None,
        idx_max: int | None = None,
        pos_exacta: int | None = None,
        dia_semana: list[int] | None = None,
        dia_mes:    list[int] | None = None,
        mes:        list[int] | None = None,
    ) -> list[dict]:
        """
        Ranking por atraso o frecuencia.
        pos_exacta: filtrar solo esa posición.
        dia_semana: lista de días 0=lun..6=dom.
        dia_mes: lista de días del mes 1..31.
        """
        idx_min, idx_max = self._resolver_rango(idx_min, idx_max)
        indices_dia = self._indices_por_dia(idx_min, idx_max, dia_semana, dia_mes, mes)
        d = self._dic(cifras)
        resultados = []
        ...
        for num, apariciones in d.items():
            if pos_exacta is not None:
                filtradas = [
                    (i, p, f) for i, p, f in apariciones
                    if p == pos_exacta and idx_min <= i <= idx_max
                    and (indices_dia is None or i in indices_dia)
                ]
            else:
                filtradas = self._filtrar_apariciones(apariciones, hasta, idx_min, idx_max)
                if indices_dia is not None:
                    filtradas = [(i, p, f) for i, p, f in filtradas if i in indices_dia]
            if not filtradas:
                continue
    ...
    def fechas_a_indices(
        ...
        if desde or hasta_fecha:
            ...
            idx_max = self.total - 1
            if desde:
                dt_desde = _parse_fecha_flexible(desde)
                for i, (fecha, _, _) in enumerate(self.historial):
                    if _parse_fecha(fecha) >= dt_desde:
                        idx_min = i
                        break
            if hasta_fecha:
                dt_hasta = _parse_fecha_flexible(hasta_fecha)
