...
def ranking(
    ...
    top: int      = Query(default=20, ge=1, le=1000),
    idx_min:     Optional[int] = Query(default=None),
    idx_max:     Optional[int] = Query(default=None),
    desde:       Optional[str] = Query(default=None),
    hasta_fecha: Optional[str] = Query(default=None),
    ...
    mes:         Optional[str] = Query(default=None),  # "1,2,12" ene=1..dic=12
):
    ...
    ms = [int(x) for x in mes.split(',')        if x.strip().isdigit()] if mes        else None
    return _motor(request).ranking(cifras, hasta, modo, top, imin, imax, pos_exacta, ds, dm, ms)
